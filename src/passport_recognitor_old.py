...
class PassportRecognitor:
    ...
    def extract_passport_data(self, image: np.ndarray):
        ...
        if passport_type == "internal":
            passport_data = self.extract_internal_passport_data(paddle_result, image)
        else:
    ...
    def extract_internal_passport_data(self, paddle_result: dict, image):
        ...
        department_code_poly = paddle_result[0]["dt_polys"][depratment_code_id]

        down_dir = self.normalize_vector(paddle_result[0]["dt_polys"][-1][3] - paddle_result[0]["dt_polys"][-1][0])

        upper_page_down_dir = down_dir = self.normalize_vector(department_code_poly[3] - department_code_poly[0])

        upper_page_right_dir = np.array([upper_page_down_dir[1], upper_page_down_dir[0]])

        pt1 = (department_code_poly[0][0], department_code_poly[0][1])
        pt2 = (int(pt1[0] + upper_page_right_dir[0] * 100), 
            int(pt1[1] + upper_page_right_dir[1] * 100))
        

        denominator = (pt2[1] - pt1[1])

        if denominator != 0:
            left_most_point = np.array([int(0), int(pt1[1] + (pt1[1] - pt2[1]) * (pt1[1]/denominator))])
        else:
            left_most_point = np.array([int(0), pt1[1]])
        
        department_border_line = [left_most_point, (left_most_point + upper_page_right_dir * 2000).astype(int)]
        
        cv2.line(image, department_border_line[0], department_border_line[1], (255, 0, 0), 3)
        cv2.circle(image, left_most_point, 3, (0, 0, 255), -1)
